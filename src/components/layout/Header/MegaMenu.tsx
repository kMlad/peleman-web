...
const MegaMenu = ({ categories, closeMegaMenu }: P): JSX.Element => {
  return (
    <>
      <section
        id="desktop"
        className={`absolute z-50 hidden w-full bg-primary  text-white md:block `}
      >
                  ...
                  {item.subitems?.map((subitem: Link) => (
                    <>
                      <div>
                        ...
                        <div className=" w-[70%] text-xs font-light">
                          {subitem.description}
                        </div>
      ...
      <div className="fixed inset-0 z-40 opacity-0" onClick={closeMegaMenu} />
    </>
  );
};
